...
def readLogs(filename):
    ...
    for species in logs:
        if species != "Plant":
            ...
            for i in range (len(logs["Rabbit"]["population"])-1):
...
# ------------------------------------------------------------------------------------------------------------------
logs_list = [readLogs('./Census_low_reprod_cost.txt'), readLogs('./Census_baseline.txt'), readLogs('./Census_high_reprod_cost.txt')]
marker_style = ['--', '-', ':']

# need to run at least max_timestep
max_timestep = 500
for key in list(logs_list[0]["Rabbit"].keys()):
    for i in range(len(logs_list)):
        logs = logs_list[i]
        indices = [i for i in range(len(logs["Rabbit"]["population"]))]
        for species in logs:
            if species != "Plant" or key == "population":
                plt.plot(indices[:max_timestep], logs[species][key][:max_timestep],marker_style[i]+species_to_color[species])
    plt.xlabel('time')        
    plt.ylabel(key)
    plt.savefig(key+'.png')
    plt.show()
